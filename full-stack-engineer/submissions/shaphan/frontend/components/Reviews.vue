...
<Navbar />
  <div class="container">
                ...
                <div class="mt-2">
                    ...
                    <div class="d-flex align-items-center justify-content-between">
                        <div class="ratings">
                          ...
                          <fa-icon v-for="i in 5 - (product?.averageStars ||0)" :key="'_'+i" :icon="['far', 'star']" regular class="text-primary"></fa-icon>
                        </div>
                    </div>
                    <div>
                      <nuxt-link to="reviews">{{ product?.reviewsCount }} Reviews</nuxt-link>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div v-for="rev in product?.reviews" :key="rev.id" class="list-group-item list-group-item-action">
    <div class="d-flex w-100 justify-content-between">
      <h5 class="mb-1">{{ rev.User?.fullName }}</h5>
    </div>
    <div class="ratings">
      <fa-icon v-for="i in rev?.stars" :key="i" icon="star" regular class="text-warning"></fa-icon>
      <fa-icon v-for="i in 5 - (rev?.stars ||0)" :key="'_'+i" :icon="['far', 'star']" regular class="text-primary"></fa-icon>
    </div>
    <p class="mb-1">{{ rev.comment }}</p>
    <div v-if="Number(rev?.User?.id||0) == Number(user?.id)" class="d-flex">
      <button class="btn btn-link border-0" @click="deleteReview(rev.id)">
        <fa-icon icon="trash" class="text-danger"> </fa-icon>
      </button>
      <button class="btn btn-link border-0" data-bs-toggle="modal" data-bs-target="#addReviewModal" @click="setCurrentReview(rev)">
