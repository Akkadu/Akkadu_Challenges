    ...
    <div class="container">
      <div class="row align-items-center justify-content-center mt-2 py-5">
        ...
        <h3 class="mb-5">Sign in</h3>
          <form method="post" autocomplete="off" autocapitalize="off" class="max" @submit.prevent="submit()">
            <div class="row">
              ...
              <div class="text-center w-100 mt-4">
                <p class="text-muted font-weight-bold">
                  No account?
                  <nuxt-link to="signup" class="text-primary ml-2">Sign Up</nuxt-link>
                </p>
              </div>
            </div>
          </form>
...
import { ValidationProvider, extend,  } from 'vee-validate';
  export default Vue.extend({
    ...
    methods: {
      ...
      async submit() {
        await this.$axios.$post('/signin', this.form).then(({success, token, user}) => {
