...
function EditCommentForm({ onChangeHandler , value, onSubmitHandler,commentId }) {
     const comment = value.find((c) => c._id === commentId)
    

    return (
        <form className='d-flex flex-column md-form justify-content-center align-items-center mb-3' onSubmit={onSubmitHandler}>
            <div>
                <fieldset className="rating" onChange={(e) => onChangeHandler({ ...value, rating: parseInt(e.target.value) })}>
            ...
            <div className='input-wrapper'>
                <input value={value.name} type='text' className='input' onChange={(e) => onChangeHandler({ ...value, name: e.target.value })} />
                <textarea value={value.content} className='input mt-3' onChange={(e) => onChangeHandler({ ...value, content: e.target.value })} />
            </div>
